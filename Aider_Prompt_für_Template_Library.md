...
Erstelle eine vollständige Kotlin-basierte Android-Bibliothek für das Projekt-Template-Management im Namespace com.scto.mcside.
Arbeite nach den Prinzipien der Clean Architecture und SOLID. Verwende ausschließlich Kotlin, Coroutines, Flows (StateFlow/SharedFlow) und injiziere Coroutine Dispatchers über den Konstruktor für maximale Testbarkeit.

## 1. Projekt-Kontext & Struktur (WICHTIG)
Nutze den beiliegenden Projekt-Tree aus der Datei McsIDE_20260510_221722.md als Referenz für die bestehende Dateistruktur.
Lege die neuen Module passend zur bestehenden Architektur im Verzeichnis core/ an. Die Struktur der neuen Module muss exakt so aussehen:
 ...
 * **TemplateDataSource**: Lädt Template-ZIP-Archive von einer remote URL (z.B. GitHub/CDN) herunter und entpackt sie on-the-fly mittels ZipInputStream in ein lokales templateStorageDir.
   *CRITICAL SECURITY REQUIREMENT:* Implementiere zwingend einen Zip-Slip-Vulnerability-Schutz! Verifiziere via canonicalPath, dass keine Dateien außerhalb des Zielverzeichnisses entpackt werden.
